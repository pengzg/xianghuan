...
function fixFork () {
    ...
    var bodyWidth = $('body').css('offsetWidth');
    //var contnetWidth = $('#nav-wrap')[0].offsetWidth;
    var contnetWidth = $('#nav-wrap').offsetWidth;
    if (bodyWidth < 1440) {
        navMarginRight = 150 - (bodyWidth - contnetWidth) / 2;
    }
    //$('#nav')[0].style.marginRight = navMarginRight + 'px';
    $('#nav').css('margin-right',navMarginRight + 'px' );
    //$('#fork-image')[0].style.width = (document.body.style.offsetWidth < 768 ? 1 : forkWidth) + 'px';
    $('#fork-image').css('width', ($('body').css('offsetWidth') < 768 ? 1 : forkWidth) + 'px');
};
fixFork();
