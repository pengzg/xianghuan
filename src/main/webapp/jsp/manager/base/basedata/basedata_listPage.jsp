...
$(function() {
	datagrid = $('#base_datatype_datagrid').datagrid({
			url : '${pageContext.request.contextPath}/base/baseDatatypeControl/dataGrid.action?bdt_status=1',
			...
			onDblClickRow:function(rowIndex,rowData){
				$('#base_data_datagrid').treegrid({url:'${pageContext.request.contextPath}/management/base/baseDataControl/treeGrid.action',queryParams:{datatype:rowData.bdt_id,bdt_isgroup:rowData.bdt_isgroup}});
			},
			frozenColumns : [ [ {
				field : 'bdt_id',
				title : '主键 ',
			 ...
			 {field : 'bdt_isgroup',title : '级别',width : 80,sortable : true, 
					formatter : function(value, row, index) {switch (value) {case '1':return '系统';case '2':return '';}}}
				] ]
		});		
	});
			...
			columns : [ [ 
						 
						{
							...
							formatter : function(value, row) {
								var str = '';
									str += sy.formatString('<img class="iconImg ext-icon-note_edit"  onclick="baseDataList_editFun(\'{0}\');" />', row.bd_id);
									str += sy.formatString('<img class="iconImg ext-icon-note_delete"  onclick="baseDataList_deleteFun(\'{0}\');"/>', row.bd_id);
								return str;
							}
						}] ],
			toolbar : [ {
	...
	function baseDataList_appendFun() {
		...
		$('<div/>').dialog({
			href : '${pageContext.request.contextPath}/management/base/baseDataControl/addPage.action?bd_datatypeid='+rows[0].bdt_id,
			width : 450,
			height : 300,
			modal : true,
	...
	function baseDataList_deleteFun(id) {
		...
		if (node) {
			$.messager.confirm('询问', '您确定要删除【' + node.bd_name + '】？', function(b) {
				if (b) {
					$.ajax({
						url : '${pageContext.request.contextPath}/management/base/baseDataControl/delete.action',
						data : {
							bd_id : node.bd_id
						},
						cache : false,
	...
	function baseDataList_editFun(id) {
		...
		var dialog = parent.sy.modalDialog({
			url : '${pageContext.request.contextPath}/management/base/baseDataControl/editPage.action?bd_id='+node.bd_id+'&bdt_isgroup='+rows.bdt_isgroup,
			width : 380,
			height : 320,
			modal : true,
					...
					<form id="searchForm">
						<table>
							<tr>
								...
								<input name="bdt_type" value="2" type="hidden">
								<td>
