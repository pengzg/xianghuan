package org.xianghuan.web.interceptors;
...
public class SecurityInterceptor implements HandlerInterceptor {
	...
	///system/loginController/doNotNeedSessionAndSecurity_loginNameComboGrid.htm
	/**
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object object) throws Exception {
		...
		String qqurl = url.substring(0,url.lastIndexOf("/")+1)+"**";
		if (excludeUrls.contains(qqurl)) {// 如果要访问的资源是不需要验证的
			return true;
		}
		
		//APP端请求不进行校验
		if(qqurl.contains("/app/")){
			return true;
		}
		
		if(qqurl.contains("/b2b/weixin/")){
			return true;
		}
